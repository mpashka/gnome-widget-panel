...
ui_click_button() {
    ...
    ui_eval "
        (async () => {
            ...
            // Virtual-device events are timestamped in MONOTONIC MICROSECONDS
            // (GLib.get_monotonic_time), NOT global.get_current_time(): in the
            // headless test session there is no event being processed, so
            // get_current_time() returns 0 (Clutter.CURRENT_TIME). Mutter then
            // never routes the injected press/motion to an actor (the whole
            // gesture is silently dropped), so a click/hold appears to do
            // nothing. A real monotonic-us timestamp makes delivery work.
            const t = () => GLib.get_monotonic_time();
            // Move the pointer to the target, yielding to the main loop between
            // and after motions. Two things make this necessary in the headless
            // session: (1) the FIRST absolute motion of a session applies only
            // the X coordinate and clamps Y to 0, so a single motion leaves the
            // pointer off-target — a second motion to the same point settles it;
            // (2) Mutter must re-pick the actor under the new pointer position
            // before the press, or the button is delivered against the stale
            // pre-motion pick and misses the target actor.
