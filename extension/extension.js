...
const FloatingMiniPanel = GObject.registerClass(class FloatingMiniPanel extends St.BoxLayout {
    constructor(sets, extensionPath) {
        ...
        // Configured plugins ----------------------------------------------
        this._plugins = PluginManager.createConfiguredPlugins(this, extensionPath);
        ...
        this._fmpQuickToggle.menu.setHeader('view-restore-symbolic', 'Mini Panel', null);
        this._autoItem = new PopupMenu.PopupImageMenuItem('Automatic', null);
        this._fmpQuickToggle.menu.addMenuItem(this._autoItem);
        this._permItem = new PopupMenu.PopupImageMenuItem('Permanent', null);
        ...
        this._fmpQuickIndicator.quickSettingsItems.push(this._fmpQuickToggle);
        ...
        this._pvConId = PANELBOX.connect('notify::visible', () => {
            if (this._state === State.AUTO) {
                if (!PANELBOX.visible) {
                    ...
                    this._timeoutId1 = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 50, () => {
        ...
        this.connect_object('notify::width', () => {
        ...
        this._meConId = Main.extensionManager.connect('extension-state-changed', (obj, ext) => {
            if (startupComplete &&
                (ext.metadata.uuid === DTP_UUID ||
                    ext.metadata.uuid === HTB_UUID)) {
                if (ext.enabled) {
                    if (this._panelHidingExts.indexOf(ext.metadata.uuid) < 0) {
                        this._disablePermanentMode(ext.metadata.uuid);
                    }
                }
                else {
                    if (this._panelHidingExts.indexOf(ext.metadata.uuid) >= 0) {
                        this._panelHidingExts.splice(this._panelHidingExts.indexOf(ext.metadata.uuid), 1);
                        if (this._panelHidingExts.length === 0) {
                            if (this.visible)
                                this._hideFloatingMiniPanel();
                            this._permItem.reactive = true;
                            Main.notify('Floating Mini Panel allowing Permanent Mode again,', 'because no panel-hiding extension is active!');
                        }
                    }
                }
            }
        ...
        this._lpConId = this._loginManager.connect('prepare-for-sleep', (obj, state) => {
    ...
    _preparePermanentMode(on) {
        if (on) {
            ...
            OVERVIEW._overview._controls._searchEntryBin.set_style(`padding-top: ${PANELBOX.height}px;`);
        }
        else {
            if (LAYOUTMANAGER._findActor(PANELBOX) === -1) {
                ...
                OVERVIEW._overview._controls._searchEntryBin.set_style(null);
            }
        }
    }
        ...
        if (this._panelHidingExts.indexOf(phext) < 0) {
            ...
            if (this._permItem.reactive) {
                if (this._state === State.ON) {
                    ...
                    Main.notify('Floating Mini Panel switched into Auto Mode,', 'because ' + phext + ' is active!');
                }
                else {
                    this._permItem.reactive = false;
                    Main.notify('Floating Mini Panel disabed Permanent Mode,', 'because ' + phext + ' is active!');
                }
            }
        }
    }
    _tmpHide() {
        ...
        this._timeoutId2 = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 5000, () => {
            if (
            // Bug fix v5
    ...
    _showFloatingMiniPanel() {
        ...
        if (this._state !== State.AUTO) {
            let priMonGeo = Utils.priMonitorGeometry();
            PANELBOX.set_position(priMonGeo.x, Math.abs(priMonGeo.y - priMonGeo.y) - PANELBOX.height);
        }
        ...
        if (setAlign) {
            ...
            if (this[this.orientStr]) {
                if (align & Alignment.CENTER)
                    rect.y = geom.y + (geom.height - rect.height) / 2;
            }
            else {
                // BUG: 'max_x / 2' which is '(geom.x + geom.width - rect.width) / 2' was used !!!
                if (align & Alignment.CENTER)
                    rect.x = geom.x + (geom.width - rect.width) / 2;
            }
        }
    ...
    destroy() {
        ...
        this._ovConId2 = null;
        this._fmpQuickIndicator.quickSettingsItems.forEach(item => item.destroy());
