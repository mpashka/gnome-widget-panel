...
| `ux` | The interaction bar every widget is designed against: use cases first, frequency-weighted steps from where the user is, actions on the object under the pointer, exact-state Undo instead of blocking confirmations, the three input routes (pointer by default, shortcuts earned by frequency), what earns a row in the handle menu, and inheriting the duties of the top bar it replaces (`docs/process/ux.md`). |
